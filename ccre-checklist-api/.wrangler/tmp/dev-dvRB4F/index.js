...
// .wrangler/tmp/bundle-g937XN/checked-fetch.js
...
// .wrangler/tmp/bundle-g937XN/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-g937XN/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
    ...
    try {
      ...
      if (request.method === "DELETE" && path.match(/^\/api\/admin\/members\/\d+$/)) {
        ...
        if (member && member.growthzone_contact_id) {
          await env.DB.prepare("INSERT OR IGNORE INTO deleted_members (growthzone_contact_id) VALUES (?)").bind(member.growthzone_contact_id).run();
        }
      ...
      if (request.method === "POST" && path === "/api/admin/templates") {
        ...
        const maxOrder = await env.DB.prepare("SELECT MAX(sort_order) as m FROM workflow_templates WHERE member_type = ?").bind(member_type).first();
      ...
      if (request.method === "POST" && path === "/api/admin/members") {
        ...
        if (body.growthzone_contact_id) {
          await env.DB.prepare("DELETE FROM deleted_members WHERE growthzone_contact_id = ?").bind(body.growthzone_contact_id).run();
        }
      ...
      if (request.method === "POST" && path.match(/^\/api\/admin\/members\/\d+\/actions$/)) {
        ...
        await env.DB.prepare("INSERT INTO member_engagement_actions (member_id, action_type, points, metadata_json) VALUES (?, ?, ?, ?)").bind(memberId, action_type, points, JSON.stringify(metadata || {})).run();
        await env.DB.prepare(`
                    UPDATE members SET 
  ...
  try {
    ...
    for (const membership of results) {
        ...
        try {
          ...
          if (contactRes.ok) {
            ...
            if (filterRes.ok) {
              ...
              const results2 = fData.Results || fData.results || [];
              if (results2.length > 0) cData = results2[0];
            }
          }
          if (cData) {
      ...
      if (existing && (email !== existing.email || phone !== existing.phone)) {
        await env.DB.prepare("UPDATE members SET email = ?, phone = ?, updated_at = datetime('now') WHERE id = ?").bind(email, phone, existing.id).run();
      }
      if (existing) {
        skipped++;
        continue;
